  ...
  >
    ...
    <span slot="footer" class="dialog-footer" v-if="isShow">
      <Button category="configuration" @click.native="handleClose"
        >取 消</Button
      >
      <Button category="news" @click.native="putVmType">确 定</Button>
    </span>
  </el-dialog>
</template>
...
import { postFileUpload, putVmType } from "@/api";
export default {
  data() {
    return {
      form: {
  ...
  mounted() {
    console.log(this.editVmTypeObj);
    this.form = this.editVmTypeObj;
    this.imageUrl = this.editVmTypeObj.image;
  },
  methods: {
    // 表单提交
    async putVmType() {
      ...
      const res = await putVmType(this.form);
      // console.log(res);
      this.handleClose();
      this.$emit("getvmType");
      // } catch (error) {
