  ...
  >
    ...
    >
      ...
      <el-form-item label="角色:" prop="role">
        <el-select v-model="formData.role" placeholder="请输入" width="100%">
      ...
      <el-form-item label="负责区域:" prop="region">
        <el-select v-model="formData.region" placeholder="请选择" width="100%">
          <el-option
      ...
      <el-form-item label="活动性质" prop="solve">
        <el-checkbox-group v-model="formData.solve">
...
<script>
import { getRegion, addPersonnel, photo } from "@/api/personnel";
export default {
  props: {
    visible: {
      type: Boolean,
  ...
  data() {
    return {
      ...
      fromRules: {
        ...
        solve: [{ required: true, message: "请选择", trigger: "change" }],
      },
      imageUrl: "",
      getRegionName: [],
      getRegionId: [],
    ...
    async isShows() {
      ...
      try {
        const res = await addPersonnel({
          ...
          mobile: this.formData.model,
          regionId: this.getRegionId[this.formData.region],
          regionName: this.getRegionName[this.formData.region],
          status: this.formData.solve,
